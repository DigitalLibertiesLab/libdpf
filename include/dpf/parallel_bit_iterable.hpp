  ...
  public:
    ...
    explicit parallel_bit_iterable(const T & t, const Ts & ...ts)
      : begin_{t.data(), ts.data()...},
        // TODO: fix forced -2 offset
        //       currenly added as output buffer has a fixed +1 due to the possibility of wrapping
        //       parallel_bit_iterable should probably be updated to use a subinterval_iterable instead of a buffer
        end_{t.data()+t.data_length()-2, (ts.data()+ts.data_length()-2)...}
    { }
