...
HEDLEY_PRAGMA(GCC diagnostic ignored "-Wignored-attributes")
        ...
        prev = curr, ++i)
    {
        ...
        {
            ++j;
            node = dpf.template traverse_exterior<I>(buf[j]);
        }
...
template <typename DpfKey,
          typename SequenceMemoizer>
auto eval_sequence_wildcard_output(const DpfKey & dpf, const sequence_recipe & recipe, SequenceMemoizer & memoizer)
{
...
template <typename DpfKey,
          typename SequenceMemoizer>
auto eval_sequence(const DpfKey & dpf, const sequence_recipe & recipe, SequenceMemoizer & memoizer, wildcard_output_tag_)  // r-value does not make sense here
{
    assert_not_wildcard_input(dpf);

    internal::eval_sequence_wildcard_output(dpf, recipe, memoizer);
}

template <typename DpfKey>
auto eval_sequence(const DpfKey & dpf, const sequence_recipe & recipe, const wildcard_output_tag_ & tag)
{
    auto memoizer = dpf::make_double_space_sequence_memoizer<DpfKey>(recipe);
    eval_sequence(dpf, recipe, memoizer, tag);
    return memoizer;
}
