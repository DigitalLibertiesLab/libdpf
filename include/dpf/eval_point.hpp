...
template <typename DpfKey,
          typename InputT,
          typename PathMemoizer>
auto eval_point_wildcard_output(const DpfKey & dpf, InputT && x, PathMemoizer && path)
{
...
template <typename DpfKey,
          typename InputT,
          typename PathMemoizer>
auto eval_point(const DpfKey & dpf, InputT && x, PathMemoizer & path, wildcard_output_tag_)
{
    assert_not_wildcard_input(dpf);

    auto tx = dpf.offset_x(x);
    internal::eval_point_wildcard_output(dpf, tx, path);
}

template <typename DpfKey,
          typename InputT>
auto eval_point(const DpfKey & dpf, InputT && x, wildcard_output_tag_)
{
    auto path = dpf::make_basic_path_memoizer<DpfKey>();
    eval_point(dpf, x, path, wildcard_output_tag);
    return path;
}
...
template <std::size_t I = 0,
          ...
          std::enable_if_t<!std::is_base_of_v<wildcard_tag_, PathMemoizer>, bool> = true>
HEDLEY_ALWAYS_INLINE
