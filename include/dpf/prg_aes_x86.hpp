...
#ifndef LIBDPF_INCLUDE_DPF_PRG_AES_X86_HPP__
#define LIBDPF_INCLUDE_DPF_PRG_AES_X86_HPP__
...
struct aes final
{
    ...
    static block_type eval(block_type seed, uint32_t pos) noexcept
    {
        ...
        #pragma GCC unroll 14-1
        for (std::size_t j = 1; j < key.rounds; ++j)
        {
            output = _mm_aesenc_si128(output, key.rd_key[j]);
        }
        output = _mm_aesenclast_si128(output, key.rd_key[key.rounds]);
    ...
    static auto eval01(block_type seed) noexcept
    {
        ...
        for (std::size_t j = 1; j < key.rounds; ++j)
        {
            output0 = _mm_aesenc_si128(output0, key.rd_key[j]);
            output1 = _mm_aesenc_si128(output1, key.rd_key[j]);
        }
        output0 = _mm_aesenclast_si128(output0, key.rd_key[key.rounds]);
        output1 = _mm_aesenclast_si128(output1, key.rd_key[key.rounds]);
...
HEDLEY_PRAGMA(GCC diagnostic pop)
        ...
        uint32_t count, uint32_t pos = 0) noexcept
    {
        ...
        for (uint32_t i = 0; i < count; ++i)
        {
            ...
            for (std::size_t j = 1; j < key.rounds; ++j)
            {
                output[i] = _mm_aesenc_si128(output[i], key.rd_key[j]);
            }
            output[i] = _mm_aesenclast_si128(output[i],
                key.rd_key[key.rounds]);
            output[i] = simde_mm_xor_si128(output[i], seed);
        }
    }
...
HEDLEY_PRAGMA(GCC diagnostic pop)
    ...
    explicit aes128_key(const simde__m128i & fixed_key)
      : rd_key{compute_round_keys(fixed_key)} { }

  private:
    rd_key_array compute_round_keys(const simde__m128i & fixed_key)
    {
        rd_key_array rd_key;
        simde__m128i x0, x1, x2;
        rd_key[0] = x0 = fixed_key;
...
HEDLEY_PRAGMA(GCC diagnostic pop)
    ...
    explicit aes256_key(const simde__m256i & fixed_key)
      : rd_key{compute_round_keys(fixed_key)} { }

  private:
    rd_key_array compute_round_keys(const simde__m256i & fixed_key)
    {
        rd_key_array rd_key;

        simde__m128i x0, x1, x2, x3;
        rd_key[0] = x0 = simde_mm256_extracti128_si256(fixed_key, 0);
        rd_key[1] = x3 = simde_mm256_extracti128_si256(fixed_key, 0);
        x2 = simde_mm_setzero_si128();
...
#endif  // LIBDPF_INCLUDE_DPF_PRG_AES_X86_HPP__
