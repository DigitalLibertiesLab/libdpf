...
struct fixedpoint
{
    ...
    {
        fixedpoint ret{0};
        ret.value = -this->integral_representation();
        return ret;
    }
    ...
    {
        fixedpoint ret{0};
        ret.value = this->integral_representation() + rhs.integral_representation();
        return ret;
    }
...
auto constexpr make_fixed_from_integral_type(IntegralType value) noexcept
{
    fixedpoint<FractionalBits, IntegralType> ret(0.0);
    ret.value = value;
    return ret;
}
...
{
    return make_fixed_from_integral_type<FractionalBits, IntegralType>(std::imaxabs(v.integral_representation()));
}
...
constexpr auto fmod(fixedpoint<FractionalBits, IntegralType> v, double modulus) noexcept
{
    ...
    return make_fixed_from_integral_type<FractionalBits, IntegralType>(v.integral_representation() % mod.integral_representation());
}
...
struct multiplies
{
        ...
        fixedpoint<FractionalBitsRHS, IntegralType> rhs) const
    {
         ...
         -> std::pair<unsigned, unsigned>
        {
            ...
            {
                return std::make_pair(FractionalBitsRHS+FractionalBitsLHS-FractionalBitsOut, FractionalBitsOut);
            }
            if constexpr (eq)
            {
                return std::make_pair((FractionalBitsLHS+FractionalBitsLHS)/2,
                                      1+(FractionalBitsLHS+FractionalBitsLHS-1)/2);
            }
            if constexpr ((Mode == use_left_arg)    // always casting to lhs
                || ((Mode == use_min_arg) &&  lt)   // lhs happens to be min
                || ((Mode == use_max_arg) && !lt))  // lhs happens to be max
            {
                return std::make_pair(FractionalBitsRHS, FractionalBitsLHS);  // = (LHS+RHS)-LHS
            };
            if constexpr ((Mode == use_right_arg)   // always casting to rhs
                || ((Mode == use_min_arg) && !lt)   // rhs happens to be min
                || ((Mode == use_max_arg) &&  lt))  // rhs happens to be max
            {
                return std::make_pair(FractionalBitsLHS, FractionalBitsRHS);  // = (LHS+RHS)-RHS
            }
            else if constexpr ((Mode == use_arg_sum) || (Mode == use_default))
            {
                return std::make_pair(0, FractionalBitsRHS+FractionalBitsLHS);// = (LHS+RHS)-(LHS+RHS)
            }
        }();
        ...
        if constexpr (std::numeric_limits<IntegralType>::digits > 32)
        {
            ...
            {
                std::cout << "1\n";
                IntegralType v = IntegralType((simde_uint128(lhs.integral_representation())*rhs.integral_representation()) << r.first);
                return make_fixed_from_integral_type<r.second, IntegralType>(v);
            }
            else
            {
                std::cout << "2: " << r.first << "\n";
        ...
        else
        {
            ...
            {
                std::cout << "3\n";
                IntegralType v = IntegralType((uint64_t(lhs.integral_representation())*rhs.integral_representation()) << r.first);
                return make_fixed_from_integral_type<r.second, IntegralType>(v);
            }
            else
            {
...
struct countl_zero_symmetric_difference<grotto::fixedpoint<FractionalBits, IntegralType>>
{
    ...
    {
        constexpr auto adjust = dpf::utils::bitlength_of_v<typename T::integral_type>-T::bits;
  ...
  : private std::numeric_limits<IntegralType>
{
  public:
    ...
    static constexpr T min() noexcept { return grotto::make_fixed_from_integral_type<FractionalBits, IntegralType>(1); }
    ...
    static constexpr T epsilon() noexcept { return is_integer ? T(0.0) : min(); }
    static constexpr T round_error() noexcept { return T(is_integer ? 0.0 : 0.5); }
    static constexpr T infinity() noexcept { return T(0.0); }
    static constexpr T quiet_NaN() noexcept { return T(0.0); }
    static constexpr T signalling_NaN() noexcept { return T(0.0); }
    static constexpr T denorm_min() noexcept { return T(0.0); }
};
