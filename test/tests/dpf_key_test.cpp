...
TEST(DpfKeyTest, HardCodedGenCheck)
{
    ...
    auto dpfargs = dpf::make_dpfargs(x, y0, y1, y2);
    auto [dpf0, dpf1] = dpf::make_dpf<dpf::prg::aes128, dpf::prg::aes128>(dpfargs, fake_root_sampler);
    ...
    ASSERT_EQ(std::get<0>(dpf0.leaf_nodes).get()[1], 0xd68266081a585266);
    ASSERT_EQ(std::get<0>(dpf0.leaf_nodes).get()[0], 0x0e975243b4820e10);
    ASSERT_EQ(std::get<0>(dpf0.leaf_nodes).get()[1], std::get<0>(dpf1.leaf_nodes).get()[1]);
    ASSERT_EQ(std::get<0>(dpf0.leaf_nodes).get()[0], std::get<0>(dpf1.leaf_nodes).get()[0]);

    ASSERT_EQ(std::get<1>(dpf0.leaf_nodes).get()[1], 0xf4b79498656fa03f);
    ASSERT_EQ(std::get<1>(dpf0.leaf_nodes).get()[0], 0x67a02ad7b73d4c3a);
    ASSERT_EQ(std::get<1>(dpf0.leaf_nodes).get()[1], std::get<1>(dpf1.leaf_nodes).get()[1]);
    ASSERT_EQ(std::get<1>(dpf0.leaf_nodes).get()[0], std::get<1>(dpf1.leaf_nodes).get()[0]);
    ...
    simde__m128i vector{0x0000000000000000, 0x00000000FFFFFFFF},
                 blinded0 = simde_mm_add_epi32(vector, std::get<2>(dpf1.leaf_nodes).get_beaver().vector_blind),
                 blinded1 = simde_mm_add_epi32(vector, std::get<2>(dpf0.leaf_nodes).get_beaver().vector_blind),
                 mulleaf0 = simde_mm_mullo_epi32(std::get<2>(dpf0.leaf_nodes).get_beaver().vector_blind, simde_mm_set1_epi32(std::get<2>(dpf1.leaf_nodes).get_beaver().output_blind)),
                 mulleaf1 = simde_mm_mullo_epi32(std::get<2>(dpf1.leaf_nodes).get_beaver().vector_blind, simde_mm_set1_epi32(std::get<2>(dpf0.leaf_nodes).get_beaver().output_blind)),
                 leaf = simde_mm_sub_epi32(simde_mm_add_epi32(std::get<2>(dpf0.leaf_nodes).get(), std::get<2>(dpf1.leaf_nodes).get()),
                                           simde_mm_add_epi32(mulleaf0, mulleaf1));

    ASSERT_EQ(blinded0[1], std::get<2>(dpf0.leaf_nodes).get_beaver().blinded_vector[1]);
    ASSERT_EQ(blinded0[0], std::get<2>(dpf0.leaf_nodes).get_beaver().blinded_vector[0]);
    ASSERT_EQ(blinded1[1], std::get<2>(dpf1.leaf_nodes).get_beaver().blinded_vector[1]);
    ASSERT_EQ(blinded1[0], std::get<2>(dpf1.leaf_nodes).get_beaver().blinded_vector[0]);
    ASSERT_EQ(leaf[1], 0x6d9edc8a6d54b590);
    ASSERT_EQ(leaf[0], 0x97147f0cbfc700b5);

    ASSERT_EQ(dpf0.wildcard_bitmask(), "100");
    ASSERT_EQ(dpf1.wildcard_bitmask(), "100");
}

TEST(DpfKeyTest, MakeDpfRandomPoint)
{
    using prg_type = dpf::prg::aes128;
    using concrete_input_type = uint8_t;
    using input_type = dpf::wildcard_value<concrete_input_type>;
    using output_type = uint32_t;
    ...
    auto dpfargs = dpf::make_dpfargs(input_type{}, dpf::utils::make_default_v<output_type0>, dpf::utils::make_default_v<output_type1>, dpf::utils::make_default_v<output_type2>);

    static constexpr auto from_integral_type = dpf::utils::make_from_integral_value<concrete_input_type>{};
    ...
    auto [dpf0, dpf1] = dpf::make_dpf<prg_type, prg_type>(dpfargs);
    auto x = dpf0.offset_x(0) + dpf1.offset_x(0);
    ...
    concrete_input_type cur = from_integral_type(0);
    for (std::size_t i = 0; i < 1ul << dpf::utils::bitlength_of_v<concrete_input_type>; ++i, ++cur, ++it0, ++it1)
    {
        if (cur == x)
        {
    ...
    ASSERT_EQ(it0, std::cend(zip0));
    ASSERT_EQ(it1, std::cend(zip1));
}
